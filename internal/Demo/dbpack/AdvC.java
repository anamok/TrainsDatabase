...
public class AdvC extends JFrame implements ActionListener {
    JPanel advcCenter = new JPanel(new BorderLayout());
    ...
    public AdvC(JPanel gui, JPanel center, String password, String user){
        passwordp = password;
        userp = user;
        ...
        advcCenter.setBackground(Color.BLACK);
        advcCenter.setBorder(new EmptyBorder(0, 20, 0, 20));
        gui.add(advcCenter, BorderLayout.CENTER);

        JPanel squish = new JPanel();
        squish.setBackground(Color.BLACK);

        JLabel headerMessage = new JLabel("Advanced Search (C)");
        ...
        JLabel ins = new JLabel("Rank the trains that are scheduled for more than one route.", SwingConstants.CENTER);
        topMessage.add(ins);

        txtheader = new JTextArea("Train_ID: | Rank: ");
        ...
        advcCenter.add(topMessage, BorderLayout.PAGE_START);
        advcCenter.add(returnText, BorderLayout.CENTER);

        advcCenter.add(formPanel, BorderLayout.PAGE_END);
        ...
        sb.append("SELECT * from advancedSearchC() AS f(train_id int, rank bigint);");
        ...
        do {
            try{
                ...
                returnRank = resultSet.getString("rank");

            } catch (SQLException r){
                    System.out.println(r);
            }
                    
                String str1 = String.format("%-10s|      ", returnTrain);
    ...
    public void actionPerformed(ActionEvent e)  {
        ...
        if (source == backButton){
            SearchDatabase agentS = new SearchDatabase(guiPush, advcCenter, passwordp, userp);
