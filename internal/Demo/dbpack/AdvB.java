...
public class AdvB extends JFrame implements ActionListener {
    JPanel advbCenter = new JPanel(new BorderLayout());
    ...
    public AdvB(JPanel gui, JPanel center, String password, String user){
        passwordp = password;
        userp = user;
        ...
        advbCenter.setBackground(Color.BLACK);
        advbCenter.setBorder(new EmptyBorder(0, 20, 0, 20));
        gui.add(advbCenter, BorderLayout.CENTER);

        JPanel squish = new JPanel();
        squish.setBackground(Color.BLACK);

        JLabel headerMessage = new JLabel("Advanced Search (B)");
        ...
        JLabel ins = new JLabel("Find routes that travel more than one rail line.", SwingConstants.CENTER);
        topMessage.add(ins);
        ...
        advbCenter.add(topMessage, BorderLayout.PAGE_START);
        advbCenter.add(returnText, BorderLayout.CENTER);

        advbCenter.add(formPanel, BorderLayout.PAGE_END);
        ...
        sb.append("SELECT * from advancedSearchB() AS f(route_id int);");
    ...
    public void actionPerformed(ActionEvent e)  {
        ...
        if (source == backButton){
            SearchDatabase agentS = new SearchDatabase(guiPush, advbCenter, passwordp, userp);
